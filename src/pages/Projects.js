...
const Projects = ({ placeholder }) => {
    ...
    const addData = (values) => {
      ...
      obj = {...values, text_az: contentAz, text_de: contentDe, text_en: contentEn, category_az: categories.name_az, category_en: categories.name_en, category_de: categories.name_de}
      obj.status = status;
    ...
    const editData = (values) => { 
      ...
      obj.status = status;
      delete obj.categories;
      dispatch(editProject({id, data: {...obj}}))
      .unwrap()
      .then((originalPromiseResult) => {
    ...
    const changeStatus = (e, id) => {
      ...
      let editedData = data.data.find(d => d.id === id)
      dispatch(switchStatusProject({...editedData, status: e}))
      .unwrap()
      .then((originalPromiseResult) => {
        successNotification("edit");
      })
    ...
    return (
      <>
        ...
        <Helmet>
          <title>{t('titles.pageName')} - {t('menu.item06')}</title>
        </Helmet>
